 ...
 */
export const splitSpoilers = (text: string): string[] => {
  ...
  if (match_start == undefined && match_end == undefined) return [text];
  if (match_start && (match_end == undefined || match_start.index < match_end.index)) {
    console.log("Recieved start token");
    const start = match_start.index;
    const end = start + match_start[0].length;
    console.log([text.slice(0, start > 0 ? start - 1 : 0), text.slice(start, end), text.slice(end + 1)]);
    return [text.slice(0, start > 0 ? start - 1 : 0), text.slice(start, end)].concat(splitSpoilers(text.slice(end + 1)));
  } else if (match_end) {
    console.log("Recieved end token");
    const start = match_end.index;
    const end = start + match_end[0].length;
    console.log([text.slice(0, start > 0 ? start - 1 : 0), text.slice(start, end), text.slice(end + 1)]);
    return [text.slice(0, start - 1), text.slice(start, end)].concat(splitSpoilers(text.slice(end + 1)));
  }
  return [text];
}
  ...
  return (tree: Root) => {
    ...
    visit(tree, 'paragraph', (parent: Paragraph, index: number | undefined, root: Parent | undefined) => {
      ...
      while (i < parent.children.length) {
        ...
        if (isText(node)) {
          let split = splitSpoilers(node.value);
